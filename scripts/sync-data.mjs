...
const admissionsSourceDir = resolve(root, "data/processed/admissions");
const admissionsPublicDir = resolve(root, "public/data/admissions");
await mkdir(admissionsPublicDir, { recursive: true });
...
const manifest = {
  ...
  datasets: [...files.filter((file) => file.endsWith(".json")).map((file) => `gsat/${file}`), ...admissionsFiles.filter((file) => file.endsWith(".json")).map((file) => `admissions/${file}`)],
...
console.log(`已同步 ${files.length + admissionsFiles.length + 3} 個前端資料檔`);
